...
class Perceptron:
    ...
    def train(self, dataset):
        ...
        training_table.field_names = ['Тренировка сети']
        ...
        while continue_training and stability_time < 100:
            ...
            epoch += 1
            if epoch % 10 == 0:
                training_table.add_row(['Эпоха {:d} завершена. Неправильных классификаций: {:d}'.format(epoch, wrong_classifications)])
        ...
        training_table.add_row(['Тренировка завершена за {:d} эпох'.format(epoch)])
        print(training_table)
        
        accuracy_training_table = PrettyTable()
        accuracy_training_table.field_names = ['Точность на тренировочных данных']
        accuracy_training_table.add_row(['{:.1f}%'.format(float(total_classifications - min_wrong_classifications) / total_classifications * 100)])
        print(accuracy_training_table)

    # удаление мертвых и коррелирующих нейронов
    def optimize(self, dataset):
        optimization_table = PrettyTable()
        optimization_table.field_names = ['Оптимизация']
        ...
        # мертвые нейроны - те, у которых значения весов не меняются в ходе обучения. При любом вводе на выход всегда только 0 либо 1.
        ...
        optimization_table.add_row(['Мертвых нейронов в А-слое: {:d}'.format(dead_neurons)])
        coef_corr = 0.7
        ...
        for i in range(len(activations) - 1):
            if not to_remove[i]:
                for j in range(i + 1, len(activations)):
                    ...
                    if abs(corr) >= coef_corr:
                        to_remove[j] = True
        correlating_neurons = to_remove.count(True) - dead_neurons

        optimization_table.add_row(['Коррелирующих нейронов в А-слое: {:d}'.format(correlating_neurons)])
        ...
        optimization_table.add_row(['После оптимизации осталось нейронов: {:d}'.format(len(self.a_layer.neurons))])
